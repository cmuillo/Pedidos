...
- **Avisos de pedidos nuevos (PWA)**: la app es instalable en Android como aplicación. En **Pedidos**, el botón **🔔 Activar avisos** habilita (con un toque, requisito de los navegadores) una **campanita** que suena al llegar un pedido nuevo, una **notificación del sistema** y un **globito rojo con el número de pedidos pendientes** sobre el icono de la app (Badging API). La preferencia queda guardada en `localStorage`.
- **Confirmaciones como modales**: eliminar un producto y cancelar un pedido muestran un **modal de confirmación dentro de la app** (en vez de `window.confirm`, que los navegadores pueden suprimir en apps instaladas).
- **Logo del negocio**: la imagen subida en **Empresa** se **redimensiona en el cliente** antes de guardarla para no exceder el límite del servidor; así el logo se guarda de forma fiable y aparece en la pantalla del cliente.
